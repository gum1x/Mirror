...
def test_slack_resolves_me_mentions_and_links(tmp_path):
    ...
    (chan / "2024-03-05.json").write_text(json.dumps([
        {"type": "message", "user": "U2", "ts": "1709675400.0", "text": "hey <@U1> ship <https://x.io|the PR>?"},
        {"type": "message", "user": "U1", "ts": "1709675472.0", "text": "lgtm, merging"},
        {"type": "message", "subtype": "channel_join", "user": "U1", "ts": "1709675480.0", "text": "joined"},
    ]), encoding="utf-8")
...
def test_slack_unknown_me_warns_and_flags_none(tmp_path):
    (tmp_path / "users.json").write_text('[{"id":"U1","profile":{"display_name":"Sam"}}]', encoding="utf-8")
    chan = tmp_path / "dm"
    chan.mkdir()
    (chan / "2024-01-01.json").write_text('[{"type":"message","user":"U1","ts":"1.0","text":"hi"}]', encoding="utf-8")
...
def test_discord_filters_types_and_flags_me_by_id(tmp_path):
    ...
    f.write_text(json.dumps({
        ...
        "messages": [
            ...
            {"id": "3", "type": "ChannelPinnedMessage", "timestamp": "2024-03-05T21:52:00.000+00:00",
...
def test_sms_sent_detection_and_timestamp(tmp_path):
    ...
    f.write_text(
        ...
        '<sms address="+15551234567" date="1709675472000" type="2" body="on my way" contact_name="Mom"/>'
        '<sms address="+15551234567" date="1709675400000" type="1" body="ok see you" contact_name="Mom"/>'
